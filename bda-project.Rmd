...
```{r Non-informative prior, only data with delays}
options(mc.cores = parallel::detectCores())
fit4 <- brm(
    delay ~ 1 + min_temperature + max_snowdepth +  max_windgust + mean_winddirection + min_vis,
    data = df_delay
...
Also other models have converged well:
...
```{r}
rhat(fit3)
stanplot(fit3, type='rhat')
```

Model 4:
```{r}
rhat(fit4)
stanplot(fit4, type='rhat')
```
...
Model 3:
```{r}
stanplot(fit3, type='neff')
```

Model 4:
```{r}
stanplot(fit4, type='neff')
```
...
```{r}
library("bayesplot")
yrep4 <- posterior_predict(fit4, draws = 500)
```
...
Model 4:
```{r}
ppc_dens_overlay(y[0:1000], yrep4[,0:1000])
```
