...
RHat stays under 1.05 so the model has converged well.
...
To check a convergence of different chains and posterior distribution, we can plot fitted model. We can see, that all chains have converged to similar values for all parameters. We can also see, that all parameters have quite similar posterior distribution (with different means of course).  

```{r}
plot(fit1)
```

To analyse effective sample size, we can plot ratio of effective samples and all samples. This means amount of draws being able to estimate true mean value of the parameter. We can see that none Neff/N values are below 0.1 and pretty large amount of them are over 0.5, which again verify that the model has converged well.

```{r}
stanplot(fit1, type='neff')
```
As conclusion, the model has converged well.
...
We can analyse a distribution of simulated draws and observed data by plotting them into the same plot. 

```{r}
#ppc_dens_overlay(y, yrep)
```

To see how different parameters are modelled, we can draw some histograms of simulated and observed delays. The same thing could be done by plotting draws from modelled and observed data to the same plot by using ppc_dens_overlay function but unfortunately this requires too much memory. 
...
To really visualise predictions, we plot predicted delays and true delays on the same plot. From the plot we can see, that although the model has converged well, it has no real prediction power. 
